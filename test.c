...
void inorderTraversal(struct rubroNegraNo *rubroNegraNo)
{
  if (rubroNegraNo)
  {
    inorderTraversal(rubroNegraNo->lado[0]);
    printf("%d  ", rubroNegraNo->valor);
    inorderTraversal(rubroNegraNo->lado[1]);
  }
  return;
}
...
int EhNegro(struct rubroNegraNo *No)
{
  return (No == NULL) || (No->cor == 0);
}

int calculaAlturaNegra(struct rubroNegraNo *H, int alturaAtual)
{
  if (H == NULL)
  {
    return 0;
  ...
  return alt + calculaAlturaNegra(H->lado[0], alt);
}

void printaAlturaNegra(struct rubroNegraNo *auxRoot)
{
  altura = 0;
  int h = 0;
  h = calculaAlturaNegra(auxRoot, h);
  printf("%d\n", h);
}
...
int main()
{
  ...
  if (pesquisaArvore(raiz, valor) != NULL)
    printaAlturaNegra(pesquisaArvore(raiz, valor));
  else
  {
